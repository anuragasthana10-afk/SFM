...
public sealed class ZohoBooksClient
{
    ...
    private readonly Dictionary<string, string> _contactCurrencyCache = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, TaxReference> _taxCache = new Dictionary<string, TaxReference>(StringComparer.OrdinalIgnoreCase);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var item in invoice.LineItems)
        {
            ...
            var taxReference = await ResolveLineItemTaxAsync(item.TaxName, item.TaxPercentage, cancellationToken);
            if (taxReference != null)
            {
                lineItem["tax_name"] = taxReference.Name;
                lineItem["tax_id"] = taxReference.Id;
            }
    ...
    private async Task EnsureContactCurrencyAsync(int contactLocalId, string contactId, string currencyCode, CancellationToken cancellationToken)
    {
        ...
        if (_contactCurrencyCache.TryGetValue(contactId, out var cachedCurrency)
            && string.Equals(cachedCurrency, currencyCode, StringComparison.OrdinalIgnoreCase))
        {
            return;
        }

        var response = await GetAsync($"contacts/{contactId}", cancellationToken);
        if (!response.TryGetProperty("contact", out var contactElement))
        {
            return;
        }

        var currentCurrency = contactElement.TryGetProperty("currency_code", out var currencyElement)
            ? currencyElement.GetString()
            : null;
        if (string.Equals(currentCurrency, currencyCode, StringComparison.OrdinalIgnoreCase))
        {
            _contactCurrencyCache[contactId] = currentCurrency ?? currencyCode;
            return;
        }
    ...
    private async Task<TaxReference> ResolveLineItemTaxAsync(
        string lineItemTaxName,
        decimal? taxPercentage,
        CancellationToken cancellationToken)
    {
        if (!string.IsNullOrWhiteSpace(lineItemTaxName))
        {
            return await EnsureTaxReferenceAsync(lineItemTaxName, cancellationToken);
        }

        if (taxPercentage.HasValue)
        {
            return await EnsureTaxReferenceByPercentageAsync(taxPercentage.Value, cancellationToken);
        }

        return null;
    }

    private async Task<TaxReference> EnsureTaxReferenceAsync(string taxName, CancellationToken cancellationToken)
    {
        ...
        if (_taxCache.TryGetValue(taxName, out var cached))
        {
            return cached;
        }

        var reference = await _referenceStore.GetTaxReferenceAsync(taxName, cancellationToken);
        if (reference != null)
        {
            _taxCache[taxName] = reference;
            return reference;
        }

        await RefreshTaxReferencesAsync(cancellationToken);

        reference = await _referenceStore.GetTaxReferenceAsync(taxName, cancellationToken);
        if (reference != null)
        {
            _taxCache[taxName] = reference;
            return reference;
        ...
        throw new InvalidOperationException($"Tax '{taxName}' was not found in Zoho Books.");
    }

    private async Task<TaxReference> EnsureTaxReferenceByPercentageAsync(decimal taxPercentage, CancellationToken cancellationToken)
    {
        var cached = _taxCache.Values.FirstOrDefault(tax => tax?.Percentage == taxPercentage);
        if (cached != null)
        {
            return cached;
        }

        var reference = await _referenceStore.GetTaxReferenceByPercentageAsync(taxPercentage, cancellationToken);
        if (reference != null)
        {
            _taxCache[reference.Name] = reference;
            return reference;
        }

        await RefreshTaxReferencesAsync(cancellationToken);

        reference = await _referenceStore.GetTaxReferenceByPercentageAsync(taxPercentage, cancellationToken);
        if (reference != null)
        {
            _taxCache[reference.Name] = reference;
            return reference;
        ...
        throw new InvalidOperationException($"Tax with percentage '{taxPercentage}' was not found in Zoho Books.");
    }

    private async Task RefreshTaxReferencesAsync(CancellationToken cancellationToken)
    {
        var response = await GetAsync("settings/taxes", cancellationToken);
        if (!response.TryGetProperty("taxes", out var taxesElement) || taxesElement.ValueKind != JsonValueKind.Array)
        {
            return;
        }

        foreach (var taxElement in taxesElement.EnumerateArray())
        {
            if (!taxElement.TryGetProperty("tax_name", out var taxNameElement))
            {
                continue;
            }

            var taxName = taxNameElement.GetString();
            if (string.IsNullOrWhiteSpace(taxName))
            {
                continue;
            }

            if (!taxElement.TryGetProperty("tax_id", out var taxIdElement))
            {
                continue;
            }

            var taxId = taxIdElement.GetString();
            ...
            if (taxElement.TryGetProperty("tax_percentage", out var percentageElement))
            {
                if (percentageElement.ValueKind == JsonValueKind.Number && percentageElement.TryGetDecimal(out var parsedPercentage))
                {
                    taxPercentage = parsedPercentage;
                }
                else if (percentageElement.ValueKind == JsonValueKind.String
                    && decimal.TryParse(percentageElement.GetString(), out var parsedStringPercentage))
                {
                    taxPercentage = parsedStringPercentage;
                }
            }

            await _referenceStore.SetTaxReferenceAsync(taxName, taxId, taxPercentage, cancellationToken);
            _taxCache[taxName] = new TaxReference
            {
                Name = taxName,
                Id = taxId,
