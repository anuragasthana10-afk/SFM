...
public sealed class ReferenceStore
{
    ...
    private const string TaxReferenceTable = "ZohoBooks_TaxReferences";
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        var sql = $@"
            ...
            END
            IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = '{TaxReferenceTable}')
            BEGIN
                CREATE TABLE {TaxReferenceTable} (
                    TaxName NVARCHAR(128) NOT NULL,
                    Location TINYINT NOT NULL,
                    TaxId NVARCHAR(100) NOT NULL,
                    TaxPercentage DECIMAL(18, 4) NULL,
                    CONSTRAINT PK_ZohoBooks_TaxReferences PRIMARY KEY (TaxName, Location)
                );
            END";
        ...
        await EnsureLocationColumnAsync(TaxReferenceTable, true, cancellationToken);
        ...
        await EnsurePrimaryKeyIncludesLocationAsync(TaxReferenceTable, "PK_ZohoBooks_TaxReferences", "TaxName", cancellationToken);
    ...
    public Task<TaxReference> GetTaxReferenceAsync(string taxName, CancellationToken cancellationToken = default)
        => GetTaxReferenceInternalAsync(taxName, cancellationToken);

    public Task<TaxReference> GetTaxReferenceByPercentageAsync(decimal taxPercentage, CancellationToken cancellationToken = default)
        => GetTaxReferenceByPercentageInternalAsync(taxPercentage, cancellationToken);

    public Task SetTaxReferenceAsync(string taxName, string taxId, decimal? taxPercentage, CancellationToken cancellationToken = default)
        => SetTaxReferenceInternalAsync(taxName, taxId, taxPercentage, cancellationToken);
    ...
    private async Task<TaxReference> GetTaxReferenceInternalAsync(string taxName, CancellationToken cancellationToken)
    {
        ...
        using (var command = CreateCommand(sql))
        {
            AddParameter(command, "@TaxName", taxName);
            AddParameter(command, "@Location", _location);
            using (var reader = await command.ExecuteReaderAsync(cancellationToken))
            {
                if (await reader.ReadAsync(cancellationToken))
                {
                    return new TaxReference
                    {
                        ...
                        Percentage = reader.IsDBNull(2) ? (decimal?)null : reader.GetDecimal(2)
                    };
                }
            }
        }
    ...
    private async Task<TaxReference> GetTaxReferenceByPercentageInternalAsync(decimal taxPercentage, CancellationToken cancellationToken)
    {
        var sql = $@"
            SELECT TOP 1 TaxName,
                   ...
                   TaxPercentage
            FROM {TaxReferenceTable}
            WHERE TaxPercentage = @TaxPercentage
              AND Location = @Location;";

        await EnsureConnectionOpenAsync(cancellationToken);
        using (var command = CreateCommand(sql))
        {
            AddParameter(command, "@TaxPercentage", taxPercentage);
            AddParameter(command, "@Location", _location);
            using (var reader = await command.ExecuteReaderAsync(cancellationToken))
            {
                if (await reader.ReadAsync(cancellationToken))
    ...
    private async Task SetTaxReferenceInternalAsync(
        string taxName,
        string taxId,
        decimal? taxPercentage,
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(taxName) || string.IsNullOrWhiteSpace(taxId))
        {
            return;
        }

        var sql = $@"
            MERGE {TaxReferenceTable} AS target
            USING (SELECT @TaxName AS TaxName, @Location AS Location) AS source
            ON target.TaxName = source.TaxName AND target.Location = source.Location
            WHEN MATCHED THEN
                UPDATE SET TaxId = @TaxId,
                           TaxPercentage = @TaxPercentage
            WHEN NOT MATCHED THEN
                INSERT (TaxName, Location, TaxId, TaxPercentage)
                VALUES (@TaxName, @Location, @TaxId, @TaxPercentage);";
        ...
        using (var command = CreateCommand(sql))
        {
            ...
            AddParameter(command, "@TaxPercentage", taxPercentage.HasValue ? (object)taxPercentage.Value : DBNull.Value);
            await command.ExecuteNonQueryAsync(cancellationToken);
        }
    }
